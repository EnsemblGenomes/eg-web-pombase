# $Id: Transcript.pm,v 1.1.1.1 2013-02-15 16:11:31 ek3 Exp $
...
sub count_go {
  ...
  # warn " OL: ", join ' * ', @{$self->species_defs->SPECIES_ONTOLOGIES || []}, "\n";
  ...
  if (my @ontologies = @{$self->species_defs->SPECIES_ONTOLOGIES || []}) {
      ...
      # Add those mapped to the gene
      $sth->execute('Gene', $self->transcript->dbID);
      $c += $sth->fetchall_arrayref->[0][0];
      # warn "SQL : $sql \n";
      # warn join ' * ', $self->transcript->translation->dbID, $self->transcript->dbID, "\n";
      # warn "Count $c ";
      return $c;
  }
  return;
